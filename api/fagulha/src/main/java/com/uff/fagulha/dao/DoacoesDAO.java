...
public class DoacoesDAO {
    ...
    public List<Doacoes> getDoacoesByUser(final Usuario usuario) {
        managerInstance.getTransaction().begin();
        List<Doacoes> doacoes = managerInstance.createNamedQuery("Doacoes.findByUsuario").setParameter("usuario", usuario).getResultList();
        managerInstance.getTransaction().commit();
        manager.fechaConexao(managerInstance);
